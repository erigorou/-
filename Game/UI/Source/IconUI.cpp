...
void IconUI::LoadTexture(const wchar_t* path)
{
	HRESULT hr = DirectX::CreateWICTextureFromFile(m_pDR->GetD3DDevice(), path, m_res.ReleaseAndGetAddressOf(), m_texture.ReleaseAndGetAddressOf());
	if (FAILED(hr))
	{
...
void IconUI::Create(DX::DeviceResources* pDR
	, const wchar_t* path
...
// -------------------------------------------------------
void IconUI::SetWindowSize(const int& width, const int& height)
{
	m_windowWidth = width;
