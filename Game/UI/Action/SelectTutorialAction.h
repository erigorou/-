// ------------------------------------------------------------------------
//
// 名前:	SelectTutorialAction
// 機能:	セレクトシーンでチュートリアルを選択したときのアクション
// 制作:	池田桜輔
//
// ------------------------------------------------------------------------
...
/// <summary>
/// セレクトシーンでチュートリアルを選択したときのアクション
/// </summary>
class SelectTutorialAction final : public IAction
{
	// -------------------------------
	// 定数
	// -------------------------------
private:
	// チュートリアルのステージ番号
	int TUTORIAL_STAGE_NO = 0;

	// アクションの実行時間
	static constexpr float ACTION_TIME = 0.25f;

	// Y軸の位置
	static constexpr float Y_POSITION = Screen::TOP + 275.0f;
...
private:
	...
	void Move(ActionParams& param)
	{
		...
		// チュートリアルを選択したときの時間を取得
		float selectTime = gameData->GetTutorialSelectTime();
	...
	void ChangeColor(ActionParams& param)
	{
		...
		// 選択ているステージによって色を変更する
		param.color *= stageIndex == TUTORIAL_STAGE_NO ?
